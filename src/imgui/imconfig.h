...
#define IM_ASSERT(_EXPR)  AB_CORE_ASSERT(_EXPR)
#else
#define IM_ASSERT(_EXPR)  SOKO_ASSERT(_EXPR)
#endif
...
#define IM_VEC2_CLASS_EXTRA                                                 \
	ImVec2(const hpm::Vector2& f) { x = f.x; y = f.y; }						\
	operator hpm::Vector2() const { return hpm::V2(x,y); }

#define IM_VEC4_CLASS_EXTRA                                                 \
	ImVec4(const hpm::Vector4& f) { x = f.x; y = f.y; z = f.z; w = f.w; }		\
	operator hpm::Vector4() const { return V4(x,y,z,w); }
