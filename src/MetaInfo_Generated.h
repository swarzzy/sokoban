...
// Generation time (UTC): 18.11.2019 12:32:48
...
struct MetaTable_EntityBehaviorType
{
    constant u32 MemberCount = 4;
    constant b32 IsEnumClass = 0;
    constant char* Name = "EntityBehaviorType";
    constant char* UnderlyingTypeName = "int";
    constant MetaEnumHint MetaHint = MetaEnumHint_NoHint;
    EntityBehaviorType values[MemberCount];
    const char* names[MemberCount];

    constant u32 _HashMapSize = 4;
    EntityBehaviorType _hashMapValues[_HashMapSize];
    u32 _hashMapIndices[_HashMapSize];
};
...
struct MetaInfo
{
    ...
    MetaTable_EntityBehaviorType EntityBehaviorType;
};
...
inline void
AddEntry_EntityBehaviorType(MetaTable_EntityBehaviorType* table, u32 enumeratorIndex, EntityBehaviorType value, const char* name)
{
    SOKO_STATIC_ASSERT(IsPowerOfTwo(MetaTable_EntityBehaviorType::_HashMapSize));
    table->values[enumeratorIndex] = value;
    table->names[enumeratorIndex] = name;

    u32 hashMask = MetaTable_EntityBehaviorType::_HashMapSize - 1;
    u32 hash = (u32)value & hashMask;

    for (u32 offset = 0; offset < MetaTable_EntityBehaviorType::_HashMapSize; offset++)
    {
        u32 index = (hash + offset) & hashMask;
        if (!(table->_hashMapIndices[index]))
        {
...
inline const char*
GetEnumName(EntityBehaviorType e)
{
    auto* table = &_GlobalStaticStorage->metaInfo->EntityBehaviorType;
    const char* result = UNKNOWN_ENUM_VALUE;
    u32 hashMask = MetaTable_EntityBehaviorType::_HashMapSize - 1;
    u32 hash = (u32)e & hashMask;
    ...
    for (u32 offset = 0; offset < MetaTable_EntityBehaviorType::_HashMapSize; offset++)
    {
        ...
        {
            u32 enumeratorIndex = table->_hashMapIndices[index];
            SOKO_ASSERT(enumeratorIndex < MetaTable_EntityBehaviorType::MemberCount);
            result = table->names[enumeratorIndex];
            break;
        }
    }
